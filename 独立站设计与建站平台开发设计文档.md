...
- Smoke Report details 和 CLI 失败消息必须脱敏 Preview Token、敏感 Query、JSON 凭据字段、R2 签名参数和 Bearer Token；归档报告必须带 `schemaVersion`，当前 `smoke-report.v2` 结构包含顶层 `summary`，用于直接读取检查数量、失败项和生产就绪结论。
