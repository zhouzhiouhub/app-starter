...
Page Builder 视觉验收可使用前台 `/visual-acceptance?viewport=desktop|mobile` 作为本地截图夹具，但必须通过 `ENABLE_VISUAL_ACCEPTANCE_FIXTURE=true` 显式开启，生产和公开预览环境保持关闭；最终签收仍以真实设计参考、浏览器截图、差异指标和结构化验收记录为准。
