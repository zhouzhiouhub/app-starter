...
- MVP 的 `POST /api/v1/translations` 只允许保存默认 Locale 单条翻译，并在响应 meta 中返回 `writeMode=created|updated`；非默认 Locale 在 `MULTI_LOCALE_ENABLED=false` 时返回 `MULTI_LOCALE_DISABLED`。
