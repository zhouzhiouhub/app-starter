...
建设一个支持高还原度设计、后台可视化管理、多语言、多市场、电商支付、数据分析的独立站建站平台。
...
```
            ...
            +-------------+-------------+
            |                           |
      Next.js (Vercel/ECS)        Admin SPA (静态)
            |                           |
            +-------------+-------------+
                          |
                   API Gateway / LB
...
| 购物 | Web → Cart API → Checkout → Stripe → Webhook → Order PAID |
...
类似 Figma / Webflow / Shopify Theme Editor 的区块编辑能力。
...
| Slug | 类型 | 说明 |
|------|------|------|
| hero-banner | section | 首屏大图 + CTA |
| product-card | block | 商品卡片 |
| video-banner | section | 视频背景 Banner |
| faq | section | 折叠 FAQ |
| spec-table | block | 参数规格表 |
| rich-text | block | 富文本内容 |
| image-gallery | block | 图片画廊 |
| cta-bar | block | 行动号召条 |
...
interface Page {
  ...
  siteId: string;
  slug: string;
  type: 'landing' | 'product' | 'blog' | 'policy';
...
```json
["page:read", "page:write", "page:publish", "product:write", "order:read", "order:refund"]
```
...
| Method | Path | 说明 |
|--------|------|------|
| GET | /api/v1/public/pages/:slug | 获取已发布 Schema |
| GET | /api/v1/public/products/:slug | 商品详情 |
| POST | /api/v1/public/cart | 加购 |
| POST | /api/v1/public/checkout | 创建订单 + PaymentIntent |
...
| 周 | 交付物 |
|----|--------|
| 1-2 | Monorepo 搭建、DB Schema、基础 API、Design Token、Ant Design 主题与 Layout 脚手架 |
| 3-4 | Page Schema + Renderer、3 个基础组件（Hero / RichText / ImageGallery） |
| 5-6 | Admin 页面列表 + Page Builder MVP、Media 上传 |
| 7 | 发布系统 + Preview + ISR |
| 8 | SEO、Staging 部署、Smoke Test |
