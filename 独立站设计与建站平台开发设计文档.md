...
- Smoke Report details 和 CLI 失败消息必须脱敏 Preview Token、敏感 Query、JSON 凭据字段、R2 签名参数和 Bearer Token；归档报告必须带 `schemaVersion`，当前 `smoke-report.v3` 结构包含顶层 `summary`、结构化失败详情、`storefrontRequestUrl` 和公开 `storefrontUrl`，用于直接读取检查数量、失败项和生产就绪结论；落盘前必须校验检查状态、通过 / 失败时间、失败错误消息和摘要新鲜度。
