...
| Webhook 路由 | 部署 `POST /api/v1/webhooks/stripe` 占位，默认返回未启用状态；仅捕获 raw body 并识别 `stripe-signature` 形状，不验签、不落库、不处理事件 | 替换为 Stripe Provider 处理逻辑 |
| 基础 API | MVP 先提供后台 Products / Orders / Payments 只读空列表占位，meta 标记关闭态、默认市场 / 币种、不可写和 Phase 2 预留 | Phase 2 开放 Product / Variant / Price / Inventory CRUD 与订单 / 支付只读运营 |
...
> **阶段边界：** MVP 不连接真实支付流程，仅预留 Stripe 配置、Webhook Endpoint、raw body 捕获契约、幂等表和订单状态机设计。生产环境必须能够安全保存密钥位，并在 `COMMERCE_ENABLED=false` 时拒绝支付创建请求。
