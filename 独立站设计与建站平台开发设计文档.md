...
- MVP 的 `GET /api/v1/translations` 支持默认 Locale 的 `page` / `limit` 分页、`namespace` 前缀筛选和 `q` 搜索，并基于当前 Tenant 页面最新草稿与已发布版本报告缺失默认 Locale 翻译 key；批量导入 / 导出只保留受保护占位契约，真实导入、预览和错误报告放到后续阶段。
...
| POST | /api/v1/translations/export | MVP 占位契约；真实导出为后续阶段 |
| POST | /api/v1/translations/import | MVP 占位契约；真实导入、预览和错误报告为后续阶段 |
