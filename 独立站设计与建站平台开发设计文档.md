...
| 基础 API | MVP 先提供后台 Products / Orders / Payments 只读空列表占位，meta 标记关闭态、默认市场 / 币种、不可写和 Phase 2 预留；商品写入、cart、checkout 和 Webhook 写入返回共享 Schema 定义的 `COMMERCE_DISABLED` 安全 details，不回显请求体、商品 ID、Webhook payload 或签名 | Phase 2 开放 Product / Variant / Price / Inventory CRUD 与订单 / 支付只读运营 |
