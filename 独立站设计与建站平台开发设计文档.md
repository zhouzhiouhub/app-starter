...
- 真实批量导出在 MVP 返回当前 Tenant、当前筛选条件下的默认 Locale JSON payload，并写入不含翻译正文的审计日志；真实批量导入仍只保留受保护占位契约，MVP 返回 `CONFLICT`，导入写入、幂等和导入审计日志放到后续阶段。
...
| POST | /api/v1/translations/import | MVP 真实导入占位契约；返回 `CONFLICT` |
