...
- MVP 的 `GET /api/v1/translations` 支持默认 Locale 的 `namespace` 前缀筛选和 `q` 搜索；批量导入 / 导出只保留受保护占位契约，真实导入、预览和错误报告放到后续阶段。
...
| GET | /api/v1/translations | 翻译列表，支持 `?locale=en-US&namespace=page.home&q=hero` |
