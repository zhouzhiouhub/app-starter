...
| Webhook 路由 | 部署 `POST /api/v1/webhooks/stripe` 占位，默认返回 `COMMERCE_DISABLED`；仅捕获 raw body 并识别 `stripe-signature` 形状，不验签、不落库、不处理事件，关闭态 details 只暴露 `resource`、`action`、`writable=false`、`writeDisabledCode=COMMERCE_DISABLED` 和 `reservedPhase=phase-2` | 替换为 Stripe Provider 处理逻辑 |
...
> **阶段边界：** MVP 不连接真实支付流程，仅预留 Stripe 配置、Webhook Endpoint、raw body 捕获契约、幂等表和订单状态机设计。生产环境必须能够安全保存密钥位，并在 `COMMERCE_ENABLED=false` 时拒绝支付创建请求；关闭态错误只返回共享 Schema 定义的安全 details，不回显请求体、商品 ID、Webhook payload 或签名。
