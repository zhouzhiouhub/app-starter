...
- `pnpm release:check -- --smoke-report artifacts/production-smoke/smoke-report.json` 统一发布证据门禁，确认生产 Smoke 与 Page Builder 视觉 accepted 证据都已齐备；需要归档时通过 `--json` 或 `--output artifacts/release/release-check.json` 生成 `release-evidence-check.v1` 机器可读报告。
- Sentry / 日志 / 指标确认无异常峰值。
