...
| 二次开发兼容 | 支持源码交付后的客户自定义           | 提供配置、主题、组件、API、后端模块、扩展应用等分层二开能力，并明确核心代码升级边界                        |
...
| 审计日志              | `Table`、`Form`、`Input`、`Select`、`Tag`          |
...
```json
{
  ...
  "chrome": {
    "header": {
      ...
      "variant": "minimal",
      "content": {
        "brand": { "label": { "defaultValue": "BrandName" }, "href": "/" },
        "navigation": [
          { "id": "home", "label": { "defaultValue": "Home" }, "href": "/" },
          {
            "id": "support",
            "label": { "defaultValue": "Support" },
            "href": "/support"
          }
        ],
        "localeSwitcher": {
          ...
          "locales": [
            { "code": "en-US", "label": { "defaultValue": "English" } }
          ]
        }
      }
    },
    "footer": {
      ...
      "content": {
        "brand": { "label": { "defaultValue": "BrandName" }, "href": "/" },
        "copyright": {
          "defaultValue": "© 2026 BrandName. All rights reserved."
        },
        "navigation": [
          {
            "id": "privacy",
...
`layout.{viewport}.sectionOrder` 为可选字段；未配置时 Renderer 按 `sections[]` 原始顺序渲染，配置后 Desktop / Mobile 可独立调整区块顺序。
...
- `props` 按组件 `schema` 校验
- 发布前运行 Schema Linter（缺失 alt、空链接、`media://` 缺失 / 非图片引用、Desktop / Mobile 画布溢出、左侧裁切和纵向压缩等告警）
...
- Section 列表标记当前端已隐藏区块，避免预览差异无法定位
- 编辑器显示 Draft saved / Unsaved draft 状态，并在 30s 空闲或窗口失焦时自动保存
- 复制 Desktop → Mobile 一键同步布局或区块顺序（可选覆盖确认）
...
| Translation 读取 | `GET /api/v1/translations` 可用，默认只返回 `en-US` 条目，支持分页和缺失 key 检查 | 返回指定 Locale 的翻译集                           |
...
- MVP 的 `POST /api/v1/translations` 只允许保存默认 Locale 单条翻译，并在响应 meta 中返回 `writeMode=created|updated`；`PATCH /api/v1/translations/:id` 只允许按当前 Tenant 更新已存在的默认 Locale 条目，并通过 Idempotency-Key 与审计日志保护；非默认 Locale 在 `MULTI_LOCALE_ENABLED=false` 时返回 `MULTI_LOCALE_DISABLED`。
- MVP 的 `GET /api/v1/translations` 支持默认 Locale 的 `page` / `limit` 分页、`namespace` 前缀筛选和 `q` 搜索，并基于当前 Tenant 页面最新草稿与已发布版本报告缺失默认 Locale 翻译 key。
- MVP 的 `POST /api/v1/translations/import/preview` 只做导入前校验和差异预览，按行返回 `create` / `update` / `duplicate` / `error` / `blocked` 与 summary，不写入数据；`POST /api/v1/translations/export/preview` 只返回当前筛选可导出数量、样例 key 和缺失 key 摘要，不生成文件。
- 真实批量导出在 MVP 返回当前 Tenant、当前筛选条件下的默认 Locale JSON payload，并写入不含翻译正文的审计日志；真实批量导入在 MVP 只允许默认 Locale 条目写入，需要 `Idempotency-Key`，复用导入预览的重复、非法和非默认 Locale 行校验，审计日志不记录翻译正文。
- MVP 的 `POST /api/v1/locales` 和 `PATCH /api/v1/locales/:id` 是受保护 Locale 写入预留接口；`MULTI_LOCALE_ENABLED=false` 时返回 `MULTI_LOCALE_DISABLED`，更新接口不回显请求体或 Locale ID，真实 Locale 持久化和状态流转放到后续阶段。
...
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| 数据库结构   | 保留 Market / Product / Variant / Price / Inventory / Order / OrderLine / Payment / WebhookEvent 表迁移                                                                                                                                                                                                                                            | 直接启用 CRUD 与交易流程                                                      |
| 环境变量     | 预留 `STRIPE_SECRET_KEY`、`STRIPE_WEBHOOK_SECRET`、`COMMERCE_ENABLED`；MVP 可留空 Stripe 密钥，但生产 readiness 会校验已配置值不是测试 key、占位值、控制字符或首尾空白，且不记录明文                                                                                                                                                               | 配置真实密钥后启用支付                                                        |
| Webhook 路由 | 部署 `POST /api/v1/webhooks/stripe` 占位，默认返回 `COMMERCE_DISABLED`；仅捕获 raw body，并识别 `stripe-signature` 是否包含可用于 Phase 2 验签的非空 `v1` 与数字 `t`，不验签、不落库、不处理事件，关闭态 details 只暴露 `resource`、`action`、`writable=false`、`writeDisabledCode=COMMERCE_DISABLED` 和 `reservedPhase=phase-2`                   | 替换为 Stripe Provider 处理逻辑                                               |
| 基础 API     | MVP 先提供后台 Products / Orders / Payments 只读空列表占位，meta 标记关闭态、默认市场 / 币种、不可写和 Phase 2 预留；商品写入、cart、checkout 和 Webhook 写入返回共享 Schema 定义的 `COMMERCE_DISABLED` 安全 details；商品、订单、支付详情 404 占位返回共享 Schema 定义的 reserved details；不回显请求体、商品 ID、资源 ID、Webhook payload 或签名 | Phase 2 开放 Product / Variant / Price / Inventory CRUD 与订单 / 支付只读运营 |
...
> **阶段边界：** MVP 不连接真实支付流程，仅预留 Stripe 配置、Webhook Endpoint、raw body 捕获契约、可验签签名形状、幂等表和订单状态机设计。生产环境必须能够安全保存密钥位，并在 `COMMERCE_ENABLED=false` 时拒绝支付创建请求；关闭态错误只返回共享 Schema 定义的安全 details，不回显请求体、商品 ID、Webhook payload 或签名。
...
MVP 预留表包含可选 `tenant_id` 与 `order_id`，便于 Phase 2 在解析事件归属后补齐租户和订单追踪；事件落库前不得把 Webhook 处理成真实支付状态变更。
...
CREATE INDEX idx_webhook_events_tenant ON webhook_events(tenant_id, created_at);
```
...
Preview Token 签发必须写入只追加的 AuditLog，记录 actor、tenant、page、site、slug 和过期时间，不记录 token 明文。

页面发布 / 回滚必须在事务内写入只追加的 AuditLog，记录 actor、tenant、page、site、slug、market、locale、发布版本 ID 或回滚源 / 目标版本 ID，不记录页面 Schema 正文。
...
`GET /api/v1/audit-logs` 为后台只读治理接口，必须校验 `audit:read`，并强制按当前 `tenantId` 查询；可按 action、actorId、targetType、targetId 过滤，不允许跨 Tenant 读取。

### 23.3 安全措施

- 密码 bcrypt（cost 12）
- JWT RS256 签名
- Refresh Token 轮换：MVP 先使用 PostgreSQL `RefreshToken` 哈希记录做原子轮换与重放检测，重放时撤销该用户未失效的刷新令牌
- Refresh Token 黑名单 / Redis 会话缓存：作为后续缓存、队列或高频会话失效优化，不作为 Phase 1 登录闭环的强依赖
...
| POST   | /api/v1/locales                     | 创建 Locale（`MULTI_LOCALE_ENABLED=false` 时返回 `MULTI_LOCALE_DISABLED`；真实持久化放到后续阶段）                   |
| PATCH  | /api/v1/locales/:id                 | 更新 Locale 显示名称 / 状态（`MULTI_LOCALE_ENABLED=false` 时返回 `MULTI_LOCALE_DISABLED`，且不回显请求内容）         |
| GET    | /api/v1/translations                | 翻译列表，支持 `?locale=en-US&page=1&limit=20&namespace=page.home&q=hero`，meta 返回分页和缺失 key 摘要              |
| POST   | /api/v1/translations                | MVP 保存默认 Locale 单条翻译；批量导入为后续阶段                                                                     |
| PATCH  | /api/v1/translations/:id            | MVP 更新当前 Tenant 下默认 Locale 的翻译值和备注；需要 `Idempotency-Key`，审计日志不记录翻译正文                     |
| POST   | /api/v1/translations/import/preview | MVP 导入前预览契约；按行返回 create / update / duplicate / error / blocked 和 summary，不写入数据                    |
| POST   | /api/v1/translations/export/preview | MVP 导出前预览契约；返回可导出数量、样例 key 和缺失 key 摘要，不生成文件                                             |
| POST   | /api/v1/translations/export         | MVP 生成当前 Tenant 默认 Locale JSON 导出 payload；非默认 Locale 请求回退默认 Locale，审计日志不记录翻译正文         |
| POST   | /api/v1/translations/import         | MVP 只允许默认 Locale 批量导入写入；需要 `Idempotency-Key`，非默认 Locale 在多语言关闭时返回 `MULTI_LOCALE_DISABLED` |
...
| GET    | /api/v1/public/products/:slug       | 商品详情占位；MVP 显式返回 `NOT_FOUND` 和 request id，不回显 slug，Phase 2 启用真实详情 |
...
| Method | Path                           | 说明                                                                                                                                                                                        |
| ------ | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | /api/v1/products               | 商品列表只读占位（后台菜单默认隐藏）；MVP 返回空列表，meta 标记 `commerceEnabled`、默认 market / currency、`writable=false`、`writeDisabledCode=COMMERCE_DISABLED`、`reservedPhase=phase-2` |
| POST   | /api/v1/products               | 商品创建占位；需要 `product:write` 与 `Idempotency-Key`，MVP 返回 `COMMERCE_DISABLED`，不回显请求体                                                                                         |
| GET    | /api/v1/products/:id           | 后台商品详情占位；MVP 返回 `NOT_FOUND`，不回显商品 ID                                                                                                                                       |
| GET    | /api/v1/products/:id/variants  | 商品变体列表只读占位；MVP 返回空列表和关闭态 meta，不回显商品 ID                                                                                                                            |
| GET    | /api/v1/products/:id/prices    | 商品定价列表只读占位；MVP 返回空列表和关闭态 meta，不回显商品 ID                                                                                                                            |
| GET    | /api/v1/products/:id/inventory | 商品库存列表只读占位；MVP 返回空列表和关闭态 meta，不回显商品 ID                                                                                                                            |
| PATCH  | /api/v1/products/:id           | 商品更新占位；需要 `product:write` 与 `Idempotency-Key`，MVP 返回 `COMMERCE_DISABLED`，不回显请求体或商品 ID                                                                                |
| GET    | /api/v1/orders                 | 订单列表只读占位；MVP 返回空列表并标记关闭态 meta                                                                                                                                           |
| GET    | /api/v1/orders/:id             | 后台订单详情占位；MVP 返回 `NOT_FOUND`，不回显订单 ID                                                                                                                                       |
| GET    | /api/v1/payments               | 支付记录列表只读占位；MVP 返回空列表并标记关闭态 meta                                                                                                                                       |
| GET    | /api/v1/payments/:id           | 后台支付详情占位；MVP 返回 `NOT_FOUND`，不回显支付 ID                                                                                                                                       |

**Webhook：**

| Method | Path                    | 说明                                                                          |
| ------ | ----------------------- | ----------------------------------------------------------------------------- |
| POST   | /api/v1/webhooks/stripe | Stripe 事件（MVP 占位；捕获 raw body 与签名形状但仍返回关闭态，Phase 2 启用） |
...
MVP 菜单默认展示 Dashboard、Pages、Design System、Media、Localization、Analytics、Users、Settings、Audit Logs。Localization 在 Phase 1 只允许查看和维护默认 `en-US` 语言配置；Products、Orders、Payments 属于 Phase 2 电商模块，Phase 1 可保留路由和权限定义，但默认通过 Feature Flag 隐藏，避免运营侧误用未完成的交易能力。
...
```typescript
type TemplateType =
  "index" | "product" | "collection" | "page" | "blog" | "policy" | "system";
...
本平台需要支持客户或实施团队拿到源码后，根据自身业务进行二次开发。二开能力必须分层设计，优先使用稳定扩展点，避免直接修改核心领域代码导致后续无法升级。

### 35.1 二次开发目标

| 目标   | 说明                                                             |
| ------ | ---------------------------------------------------------------- |
| 可改   | 客户可以修改主题、组件、页面、后台菜单、接口集成和业务规则       |
| 可控   | 二开必须经过权限、Feature Flag、测试、审计和发布流程             |
| 可升级 | 常规二开不应阻断主版本升级，核心 Fork 需要明确风险               |
| 可隔离 | 客户自定义代码与平台核心代码有目录、接口和依赖边界               |
| 可交付 | 源码、环境、文档、示例、API Contract、迁移脚本和测试基线一起交付 |

### 35.2 二开分层模型

| 层级 | 名称                  | 推荐程度 | 适用场景                                                 | 升级兼容性 |
| ---- | --------------------- | -------- | -------------------------------------------------------- | ---------- |
| L1   | 配置级二开            | 推荐     | 品牌色、域名、市场、语言、SEO、Feature Flag              | 高         |
| L2   | Theme / Template 二开 | 推荐     | 改页面结构、Header/Footer、区块组合、主题设置            | 高         |
| L3   | 组件级二开            | 推荐     | 新增前台 Section / Block、Admin 表单组件                 | 中高       |
| L4   | 集成级二开            | 推荐     | 支付、物流、税费、Analytics、ERP、CRM、Webhook           | 中高       |
| L5   | 后端模块二开          | 谨慎     | 新领域、新业务流程、新数据表、新 API                     | 中         |
| L6   | 核心 Fork             | 不推荐   | 修改 Identity、Publish、Renderer、Order 状态机等核心逻辑 | 低         |

规则：

- L1-L4 必须作为优先二开方式。
- L5 需要新增领域边界、权限、API Contract、迁移和测试。
- L6 必须在交付文档中标记为 Fork，不承诺无冲突升级。
...
├── apps/
│   ├── web/
│   │   └── src/custom/             # 前台站点客户自定义入口
│   └── admin/
│       └── src/custom/             # 后台菜单、页面、表单扩展入口
├── services/
│   └── api/
│       └── src/custom/             # 客户后端模块、Provider、事件订阅
├── packages/
│   ├── custom-components/          # 前台自定义 Section / Block
│   ├── custom-admin/               # 后台自定义页面与扩展组件
│   ├── extension-sdk/              # 扩展开发 SDK 与类型
│   └── integration-adapters/       # 支付、物流、税费、ERP 等适配器
├── themes/
│   └── custom/                     # 客户主题包
├── extensions/
│   └── custom-apps/                # App / Extension Platform 扩展
└── docs/
    └── development/                # 二开指南、API 文档、升级说明
```
...
| 扩展点                  | 位置                                               | 能力                                        |
| ----------------------- | -------------------------------------------------- | ------------------------------------------- |
| Component Registry      | `packages/renderer` / `packages/custom-components` | 注册新的前台 Section / Block                |
| Theme Package           | `themes/custom`                                    | 新增模板、主题设置、Locale Bundle           |
| Admin Route Registry    | `apps/admin/src/custom`                            | 新增后台菜单、页面、表单和列表              |
| API Module Registry     | `services/api/src/custom`                          | 注册 NestJS 模块、Controller、Provider      |
| Domain Event Subscriber | `services/api/src/custom`                          | 订阅 `order.paid`、`page.published` 等事件  |
| Integration Adapter     | `packages/integration-adapters`                    | 新增支付、物流、税费、Analytics、ERP 适配器 |
| Webhook Subscription    | `extensions/custom-apps`                           | 订阅平台事件并推送到外部系统                |
| Feature Flag            | `packages/schema` / API                            | 控制客户定制功能启停                        |
| API Scope               | Identity / Extension                               | 限制自定义模块访问范围                      |

### 35.5 自定义组件规范

自定义前台组件必须声明 Props Schema、默认值、响应式配置、埋点和 i18n Key。
...
- 不允许组件直接请求 Admin API 或读取未发布数据。
- 不允许组件内硬编码 Tenant / Site / Market。
- 图片、视频必须走 MediaAsset。
- 文案字段应支持 `i18nKey` + 默认值。
- 组件必须提供 Desktop / Mobile 示例、视觉回归基线，并在 `docs/development/page-builder-visual-acceptance.json` 中留存带安全相对图片路径、且能解析到非空图片文件的验收记录。

### 35.6 自定义后端模块规范

自定义后端模块必须通过明确的 Module Manifest 注册，不得直接修改核心模块内部实现。

```typescript
interface CustomApiModuleManifest {
  name: string;
  version: string;
  owner: string;
...
- 必须声明权限 Scope 和 Feature Flag。
- 必须通过 Tenant Context 访问数据。
- 必须提供 migration、rollback 或不可回滚说明。
- 必须提供 API Contract 和错误码。
- 订阅领域事件时必须幂等。
- 不允许绕过 Order、Payment、Publish、Identity 的领域服务直接写核心表。

### 35.7 集成适配器规范

支付、物流、税费、邮件、ERP、CRM、Analytics 等外部系统通过 Adapter 接口集成。

| 适配器            | 接口要求                                             |
| ----------------- | ---------------------------------------------------- |
| PaymentProvider   | createIntent、refund、handleWebhook、verifySignature |
| ShippingProvider  | quoteRates、createShipment、trackShipment            |
| TaxProvider       | quoteTax、validateAddress                            |
| EmailProvider     | sendTemplate、renderPreview                          |
| AnalyticsProvider | track、identify、flush                               |
| ERPAdapter        | syncProduct、syncInventory、syncOrder                |
...
| --------- | ---------------------------------------------------- |
| 配置级    | 直接随版本升级                                       |
| Theme     | 通过 Theme Manifest 的 `schemaVersion` 和迁移器升级  |
| 组件级    | 通过 Props Schema 兼容矩阵升级                       |
| API 集成  | 通过 API Version 和 Contract Test 升级               |
| 后端模块  | 通过 Module Manifest、Migration 和 Feature Flag 升级 |
| 核心 Fork | 需要人工 rebase / merge，不承诺自动兼容              |
...
| 交付物             | 说明                                         |
| ------------------ | -------------------------------------------- |
| Source Code        | Monorepo 全量源码，不含密钥                  |
| Setup Guide        | 本地启动、环境变量、依赖安装、数据库初始化   |
| Architecture Guide | 领域边界、目录结构、数据流、发布流           |
| Development Guide  | 二开方式、扩展点、编码规范、测试规范         |
| API Docs           | OpenAPI / Webhook Payload / Error Codes      |
| Theme Docs         | Theme Package、Section、Block、Settings 示例 |
| Extension Docs     | Module Manifest、API Scope、事件订阅示例     |
| Migration Docs     | DB / Schema / Theme / Translation 迁移说明   |
| Test Baseline      | 单测、集成测试、E2E、视觉回归示例            |
| Deployment Guide   | Vercel、R2、API、DB、Redis、CI/CD 部署说明   |

### 35.10 二开禁止事项

- 禁止在自定义组件中读取 Admin Token、Draft Schema 或敏感环境变量。
- 禁止自定义模块跨租户查询或写入数据。
- 禁止绕过 Payment Provider 直接修改支付状态。
- 禁止绕过 Order Service 直接修改订单状态。
- 禁止绕过 Publish Service 直接改线上 Snapshot。
- 禁止在自定义代码中硬编码密钥、域名、租户 ID。
- 禁止直接修改已发布历史版本数据。
...
| Visual Regression | 核心区块 95% 还原、Desktop / Mobile 截图、结构化验收记录 | Phase 1  |
...
Page Builder 视觉验收可使用前台 `/visual-acceptance?viewport=desktop|mobile` 作为本地截图夹具，也可追加 `component=<hero-banner|rich-text|image-gallery|cta-bar|faq|spec-table>` 做单区块截图；该夹具必须通过 `ENABLE_VISUAL_ACCEPTANCE_FIXTURE=true` 显式开启，生产和公开预览环境保持关闭；本地可通过 `pnpm visual:capture:fixture` 执行 build、启动夹具、截图和停服务流程；CI 可通过 `Page Builder Visual` workflow 运行视觉脚本并上传 `page-builder-visual-fixture-<run_number>` fixture 截图 artifact，作为截图链路回归证据；补齐真实设计参考图后通过 `pnpm visual:measure -- --write --require-complete` 写回视觉指标；最终签收仍以真实设计参考、浏览器截图、差异指标和结构化验收记录为准。
...
- Admin 登录、页面编辑、预览、发布 Smoke Test；发布链路可用 `pnpm smoke:publish` 验证 Draft Save、Preview Token、Public Preview、Web Preview、API Publish、API Rollback、Preview / Publish / Rollback Audit Logs、Public Page、Storefront HTML 与 ISR Revalidate。
- R2 媒体访问、媒体列表筛选和 CDN 缓存检查；生产 smoke 可设置 `SMOKE_REQUIRE_R2_UPLOAD=true` 强制验证 Presigned URL、真实 PUT 上传和生产 CDN URL。生产 CDN URL 不得使用 `example` / `test` / `invalid` / 本地 / 私网域名，这些只允许作为文档占位。
- Smoke Report details、API HTTP 失败消息、网络异常、上传失败消息和 CLI 失败消息必须脱敏 Preview Token、敏感 Query、JSON 凭据字段、R2 签名参数和 Bearer Token；这些失败消息还必须归一化控制字符并限制长度；重验证诊断保留 `pathCount` / `tagCount` 总数，`paths` / `tags` / `missingPaths` / `missingTags` 只写脱敏、限长后的安全样本；回滚报告详情中的版本 ID 和标题也只写脱敏、限长后的安全样本；Identity 诊断只记录 JWT 私钥 / 公钥是否配置、能否解析为 PEM、能否完成 RS256 配对验签，不记录密钥内容；归档报告必须带 `schemaVersion`，当前 `smoke-report.v3` 结构包含顶层 `summary`、结构化失败详情、`storefrontRequestUrl` 和公开 `storefrontUrl`，用于直接读取检查数量、失败项和生产就绪结论；归档回看入口必须支持读取最近归档或指定报告，并输出 R2 / CDN、Admin 静态托管和发布链路的可追踪摘要、失败详情与修复建议；Production Smoke CI 必须在 smoke 请求前运行 `release:preflight` 并上传 `production-smoke-preflight.v1` 的 `release-preflight-<run_number>` 预检 artifact，保证生产配置或证据输入失败时也有结构化失败留痕；Production Smoke CI 必须上传报告 artifact 与 `release-evidence-check.v1` 组合门禁 artifact，并可在提供视觉 artifact 名称和 run id 时下载 Page Builder Visual 证据到 `reports/visual/page-builder-fixture` 后再运行组合门禁，在组合门禁日志中输出 release readiness checklist，且可在提供 release tag、rollback target 和视觉 artifact 输入时生成 release notes artifact；Release Checklist 中记录 workflow run、artifact、回看输出、`pnpm smoke:release-check`、`pnpm release:check` 发布证据校验结果和回滚目标；落盘前必须校验检查状态、通过 / 失败时间、失败错误消息和摘要新鲜度。
- 生产部署与回滚操作以 `infra/README.md` runbook 为准；该 runbook 必须覆盖 Vercel 前台、Admin 静态托管、独立 Node API、PostgreSQL 迁移、Redis、Cloudflare R2 / CDN、生产环境变量矩阵、Production Smoke、release handoff、release notes 和 rollback target。
- `COMMERCE_ENABLED=false`、`MULTI_LOCALE_ENABLED=false` 关闭态检查。
- `pnpm visual:acceptance -- --checklist` 输出 Page Builder 六个核心区块 Desktop / Mobile 的逐项证据缺口；真实设计 PNG 可通过 `pnpm visual:references -- --source-dir docs/visual/page-builder-references --write --require-complete` 导入 manifest 并重置旧指标；`pnpm release:check -- --checklist --smoke-report artifacts/production-smoke/smoke-report.json` 输出生产 Smoke、视觉签收和 release notes 准备状态；`pnpm release:check -- --smoke-report artifacts/production-smoke/smoke-report.json` 统一发布证据门禁，确认生产 Smoke 与 Page Builder 视觉 accepted 证据都已齐备；需要归档时通过 `--json` 或 `--output artifacts/release/release-check.json` 生成 `release-evidence-check.v1` 机器可读报告；组合门禁 ready 后通过 `pnpm release:notes` 生成包含 release tag、workflow run、artifact、公开 storefront URL 与 rollback target 的最终发布记录。
- Sentry / 日志 / 指标确认无异常峰值。
...
| 二次开发            | 客户或实施团队在源码交付后基于扩展点进行定制开发           |
| Extension Point     | 平台预留的稳定扩展入口，如组件注册、Admin 路由、事件订阅   |
| Adapter             | 外部系统适配器，用统一接口接入支付、物流、税费、ERP 等服务 |
| Core Fork           | 直接修改平台核心代码的分叉方式，升级兼容性最低             |
...
| MULTI_LOCALE_DISABLED    | 409  | 多语言能力未启用，禁止创建、更新或发布非默认 Locale |
...
```bash
# Database
# 生产 DATABASE_URL / REDIS_URL 不允许控制字符或首尾空白；REDIS_URL 应使用 rediss://。
...
JWT_PRIVATE_KEY=...
JWT_PUBLIC_KEY=...
# 本地 seed 默认管理员仅限非生产；生产必须显式设置非默认值。邮箱必须有效，密码必须为 8 到 128 字符且不含控制字符。
SEED_ADMIN_EMAIL=admin@example.com
SEED_ADMIN_PASSWORD=ChangeMe123!

# Runtime Boolean Gates：只接受 true/false、1/0、yes/no、on/off，拼写错误必须失败
...
# MVP 可留空；如生产环境已配置，readiness 会校验格式安全性且不记录明文。
STRIPE_SECRET_KEY=
STRIPE_WEBHOOK_SECRET=

# Cloudflare R2
# 生产环境 R2_ACCOUNT_ID 必须是 DNS 安全标签；R2_BUCKET 必须是 3 到 63 字符的安全 bucket 名称；凭据和 region 不得包含空白或控制字符。
R2_ACCOUNT_ID=...
R2_ACCESS_KEY_ID=...
R2_SECRET_ACCESS_KEY=...
R2_BUCKET=...
R2_REGION=auto
# 生产环境必须替换为真实公开 CDN Origin；示例、test、invalid、本地和私网域名不算生产可用。
MEDIA_CDN_BASE_URL=https://cdn.example.com
MEDIA_EXTERNAL_URL_HOSTS=
MEDIA_UPLOAD_BASE_URL=

# Analytics
# Provider ID 必须精确匹配厂商格式、最多 64 字符，且不带首尾空白或控制字符。
GTM_CONTAINER_ID=GTM-...
GA4_MEASUREMENT_ID=G-...
CLARITY_PROJECT_ID=...

# App
# 生产运行时和 smoke URL 不允许账号密码、query、fragment、控制字符或首尾空白；API_URL 可为 origin 或 /api/v1，WEB_URL / ADMIN_URL 只允许 origin。
WEB_URL=https://example.com
ADMIN_URL=https://admin.example.com
API_URL=https://api.example.com

# Storefront ISR Revalidation
# 生产环境必须使用不超过 1024 字符、不包含控制字符且不带首尾空白的非空密钥；API、Web 和 smoke readiness 采用同一安全边界。
STOREFRONT_REVALIDATE_SECRET=...
# 必须使用前台 origin 或精确的 /api/revalidate endpoint，不能带首尾空白、账号密码、query、fragment 或控制字符。
STOREFRONT_REVALIDATE_URL=https://example.com/api/revalidate
STOREFRONT_REVALIDATE_TIMEOUT_MS=5000

# Preview Token
# 生产环境必须使用 32 到 1024 字符、不包含控制字符且不带首尾空白的签名密钥；PREVIEW_TOKEN_PREVIOUS_SECRET 如配置也必须满足同样边界。
PREVIEW_TOKEN_SECRET=...
PREVIEW_TOKEN_PREVIOUS_SECRET=
PREVIEW_TOKEN_TTL_SECONDS=3600

# Smoke Test（可选覆盖）
# 登录配置发起请求前校验：邮箱必须有效，密码必须为 8 到 128 字符且不含控制字符；Tenant Slug 必须为小写字母、数字和连字符，且不能以连字符开头或结尾。
SMOKE_ADMIN_EMAIL=admin@example.com
SMOKE_ADMIN_PASSWORD=...
SMOKE_TENANT_SLUG=default
SMOKE_REQUIRE_R2_UPLOAD=true
SMOKE_REQUIRE_REVALIDATION=true
# 相对 .json 路径，且必须位于 tmp/、reports/、artifacts/ 或 .tmp/ 下
