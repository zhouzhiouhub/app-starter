...
该脚本会登录默认管理员，先验证 `COMMERCE_ENABLED=false`、`MULTI_LOCALE_ENABLED=false` 的关闭态和媒体上传目标，再发布一个唯一 slug 的测试页，验证公共页面 API、前台 HTML、`robots.txt`、`sitemap.xml` 和 404/noindex 是否读取到同一份已发布内容并满足 SEO 发布门禁。生产环境如果要强制验证 R2 Presigned URL，可设置 `SMOKE_REQUIRE_R2_UPLOAD=true`。若只想验证发布与前台读取、暂不强制 ISR 回调，可临时设置：
