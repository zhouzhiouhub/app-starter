...
`pnpm smoke:dispatch` 在预览和 `--require-complete` 失败时也会显示首个缺失 input 及替换原因，方便先补齐最前面的 production smoke 证据输入。
...
3. 按 [生产环境变量核对清单](./docs/development/production-env-checklist.md) 和 `infra/README.md` 准备前台 Vercel、API 独立 Node 服务、Admin 静态托管、云端 PostgreSQL / Redis、R2 / CDN 和环境变量；确认 CDN 不是 `example` / `test` / `invalid` / 本地 / 私网域名。
