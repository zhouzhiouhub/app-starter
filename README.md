...
4. 继续完善 Translation Key 管理后台体验：导入历史与修复队列的键级操作提示、批量草稿预览后的差异定位和默认 Locale 导入前的最终确认摘要；非默认 Locale 仍保持关闭态。
