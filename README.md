...
- 证据请求摘要：`pnpm smoke:request`、`pnpm release:evidence-request`、`pnpm release:requests` 和 `pnpm project:status -- --summary` 会在 Production Smoke 输入仍有 placeholder 时显示第一条缺失 input 的替换原因；`pnpm release:requests` 还会在 bundle 摘要里显示第一条缺失视觉参考图及对应预览截图摘要。
