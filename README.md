...
4. 继续完善 Translation Key 管理后台体验：导入预览结果的批量修复入口、历史回放与当前筛选的定位校准、导入确认后的成功 / 失败分支复盘；非默认 Locale 仍保持关闭态。
