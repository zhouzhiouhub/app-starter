...
最终发布前可运行 `pnpm release:check -- --checklist --smoke-report artifacts/production-smoke/smoke-report.json` 查看生产 Smoke、Page Builder 视觉证据和 release notes 的准备状态；去掉 `--checklist` 后同一命令可作为统一门禁，它会同时校验生产 Smoke 发布证据和 Page Builder 视觉 accepted 证据。需要归档时追加 `--json` 输出机器可读结果，或追加 `--output artifacts/release/release-check.json` 写入 `release-evidence-check.v1` JSON artifact。当前缺真实设计参考和生产 smoke artifact 时，该命令会阻塞发布。
