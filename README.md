...
需要一次性生成本地交接包时，可运行 `pnpm release:handoff -- --smoke-report artifacts/production-smoke/smoke-report.json --visual-artifact-dir reports/visual/page-builder-fixture`。它会从同一次 handoff 运行写出 `artifacts/release/preflight.json`、`artifacts/release/preflight.md`、`artifacts/release/release-check.json`、`artifacts/release/release-check.md`、`artifacts/release/project-status.json` 和 `artifacts/release/project-status.md`；终端摘要会打印 first two next actions、预览 first structured next-action steps，并把剩余动作指向 `artifacts/release/project-status.md` 的完整清单；证据 blocked 时仍会写出交接材料，只有 release evidence 与 preflight 都 ready 时，加 `--require-ready` 才会放行。
