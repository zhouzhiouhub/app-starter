...
GitHub Actions 里新增了 `Page Builder Visual` workflow，会在相关 PR、main 推送或手动触发时运行 `pnpm test:visual`、`pnpm visual:acceptance -- --checklist --output reports/visual/page-builder-fixture/visual-acceptance-report.json`、`pnpm visual:measure` 和 `pnpm visual:capture:fixture -- --output-dir reports/visual/page-builder-fixture --report reports/visual/page-builder-fixture/visual-capture-report.json`，并上传 `page-builder-visual-fixture-<run_number>` artifact。该 artifact 带有 fixture 截图、`visual-capture-report.json` 截图清单和 `visual-acceptance-report.json` 结构化验收状态，只证明 fixture 截图链路可持续回归，不替代真实设计参考、差异指标和 `pnpm visual:acceptance -- --require-accepted` 最终签收。
