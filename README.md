...
`visual.artifactCheck.referenceImport.missingReferences` 会保留缺失参考图路径清单；终端摘要预览第一条缺失路径，`project-status.md` 在缺图时新增 `Missing Visual References` 小节，Markdown 报告列出可读路径，完整明细仍以 `visual-reference-import-report.json` 为准。
