...
- 生产 smoke 报告 `SMOKE_REPORT_PATH` 归档、`smoke-report.v3` 摘要、失败详情脱敏、`pnpm smoke:report` 最近归档 / 指定报告回看入口、Smoke Markdown 回看清单、`pnpm smoke:release-check` 发布证据强校验，以及 GitHub Actions `Production Smoke` artifact 留存和 Release Checklist。
...
4. 在真实生产配置下触发 GitHub Actions `Production Smoke`，传入 Page Builder Visual artifact 名称和 run id，把 smoke、Smoke Markdown 回看清单、release evidence JSON / Markdown、project status 和 visual artifact、`pnpm smoke:report` 输出、`pnpm smoke:release-check`、带 `--visual-artifact-dir` 的 `pnpm release:check` 结果和回滚目标写入 `pnpm release:notes` 生成的发布记录。
