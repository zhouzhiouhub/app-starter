...
4. 继续完善 Translation Key 管理后台体验：缺失队列自动推进保存下一条、导入结果历史保留和批量修复后的完成确认；非默认 Locale 仍保持关闭态。
