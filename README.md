...
Page Builder Visual artifact check 还会要求 `visual-reference-import-report.json` 与 `visual-reference-import-report.md` 同时保留，并校验 JSON schema、artifact-local manifest 路径、默认参考图目录、`sourceDirStatus`、状态和 missing/update 计数一致；运行时可追加 `--output reports/visual/page-builder-fixture/visual-artifact-check-report.json` 留存机器可读完整性检查结果。
