...
export async function runPageBuilderVisualReferenceImportCli(args) {
  ...
  try {
    ...
    } else if (config.missingPaths) {
      for (const missing of artifact.missing) {
...
function printHelp() {
  ...
  pnpm visual:references -- --missing-paths
  pnpm visual:references:missing
...
Options:
  ...
  --missing-paths        Print only missing expected PNG paths, one per line.
...
Evidence:
  ...
  measured again before sign-off. Use pnpm --silent visual:references:missing
  when the output will be copied as a plain missing-path list.`);
}
