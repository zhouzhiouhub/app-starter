...
import { readStripeSecretAction } from "./smoke-readiness-commerce-actions.mjs";
import {
  readJwtAction,
...
const maxReadinessActionLength = 520;
...
function readBlockerActions(blocker) {
  ...
  if (blocker.area === "deployment.api") {
    return [createAction(blocker.area, readDeploymentAction(blocker))];
  }

  if (blocker.area === "database.url") {
    return [createAction(blocker.area, readDatabaseUrlAction(blocker))];
  }

  if (blocker.area === "database.migrations") {
    return [createAction(blocker.area, readMigrationAction(blocker))];
  }

  if (blocker.area === "cache.redis") {
    return [createAction(blocker.area, readRedisAction(blocker))];
  }
  ...
  if (blocker.area === "media.r2") {
    return [createAction(blocker.area, readR2Action(blocker))];
  }

  if (blocker.area === "media.cdn") {
    return [createAction(blocker.area, readCdnAction(blocker))];
  }

  if (blocker.area === "media.external-hosts") {
    return [createAction(blocker.area, readExternalHostsAction(blocker))];
  }

  if (blocker.area === "analytics.enabled") {
    return [createAction(blocker.area, readAnalyticsEnabledAction(blocker))];
  }

  if (blocker.area === "analytics.consent") {
    return [createAction(blocker.area, readAnalyticsConsentAction(blocker))];
  }

  if (blocker.area === "analytics.provider") {
    return [createAction(blocker.area, readAnalyticsProviderAction(blocker))];
  }
  ...
  if (
    ...
    return [createAction(blocker.area, readFeatureFlagAction(blocker))];
  }

  if (
    blocker.area === "commerce.stripe" ||
    (typeof blocker.area === "string" &&
      blocker.area.startsWith("commerce.stripe."))
  ) {
    return [createAction(blocker.area, readStripeSecretAction(blocker))];
  }
  ...
  if (
    ...
    if (blocker.area === "identity.jwt.pair") {
      return [createAction(blocker.area, readJwtAction(blocker))];
    }
  ...
  if (blocker.area === "revalidation") {
    return [createAction(blocker.area, readRevalidationGateAction())];
  }

  if (blocker.area === "preview.secret") {
    return [createAction(blocker.area, readPreviewSecretAction(blocker))];
  }
  ...
  if (blocker.area === "revalidation.secret") {
    return [createAction(blocker.area, readRevalidationSecretAction(blocker))];
  }

  if (blocker.area === "revalidation.url") {
    return [createAction(blocker.area, readRevalidationUrlAction(blocker))];
  }

  if (blocker.area === "report.path") {
    return [createAction(blocker.area, readReportPathAction(blocker))];
  }
...
function createAction(area, action) {
  return {
    action: formatSmokeText(readActionMessage(action), {
      fallback:
        "Review the production readiness blocker and add a specific remediation action.",
      maxLength: maxReadinessActionLength,
    }),
