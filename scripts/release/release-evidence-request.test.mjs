...
test("release evidence request Markdown combines blocked evidence handoffs", async () => {
  ...
  const requestsManifestOutputPath = `${root}/release-requests-manifest.json`;
  ...
  try {
    ...
    assert.match(
      ...
      new RegExp(
        `Page Builder design request: \`pnpm visual:references:request -- --output ${escapeRegExp(
          ...
          visualTableOutputPath,
        )} --json-output ${escapeRegExp(visualJsonOutputPath)}\``,
      ),
    );
    assert.match(
      markdown,
    ...
    assert.match(
      markdown,
      new RegExp(
        `Export manifest output: \`${escapeRegExp(visualJsonOutputPath)}\``,
      ),
    );
